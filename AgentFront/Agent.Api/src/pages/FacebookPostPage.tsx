          ...
          <form onSubmit={handleGetRecommendation} className="space-y-4">
            ...
            <div>
              <SelectionGrid
                ...
                selectedId={selectedImageUrl ? savedImages.find(img => img.url === selectedImageUrl)?.id : null}
                onSelect={(id) => {
                  const img = savedImages.find(img => img.id === id);
                  if (img) {
                    setSelectedImageUrl(img.url);
                    setSelectedImageDockerUrl(img.dockerUrl);
                  }
                }}
                onDeselect={() => {
                  setSelectedImageUrl("");
                  setSelectedImageDockerUrl("");
                }}
                ...
                selectedMessage="Image selected. Your post will include this image."
                allowDeselect={true}
                showLabel={false}
              />
            </div>
